...
Deep technical documentation for the OKF-CV pipeline. For a high-level overview, quick start, and self-refresh instructions, see the [main README](../README.md).
...
- **Dynamic Section Ordering:** Resume section order is data-driven and depends on the `resume_style` key. US style uses `DEFAULT_SECTION_ORDER` in `renderers/resume_common.py` (Summary → Technical Skills → Projects → Professional Experience → Education → Spoken Languages). German style uses `GERMAN_STYLE_SECTION_ORDER` in `renderers/resume_latex_german.py` / `renderers/resume_reportfallback_german.py` (Summary → Professional Experience → Education → Technical Skills → Spoken Languages). German style has no separate Projects section — the 3 JD-aligned projects are folded into Professional Experience as `project_bullets` under an "Independent Data Engineering & Professional Development" entry (rendered in `name --- [GitHub] --- summary` format with quantified metrics), plus a 4th plain-text bullet for other skills/tools. The entry date ends at April 2025 (candidate is now studying economics). The title uses a concrete role (e.g., `Data Engineer`, `Analytics Engineer`) — never `Architect`/`Lead`/`Manager`. Required for German market applications.
- **GitHub Link Integration:** Project entries render as `[GitHub]` hyperlinks (clickable in PDF viewers) pointing to the specific repository `repo_url` from the portfolio frontmatter. The renderer inserts `\href{repo_url}{[GitHub]}` in LaTeX mode and `<a href="repo_url">[GitHub]</a>` in ReportFallback mode. Bare profile URLs are not used — each project links to its actual repository.
...
- **Windows File-Lock Recovery:** Folder moves are wrapped in a 3-retry loop with `gc.collect()` + 0.5s delay between attempts to release lingering file handles (`[WinError 32]`). If all rename attempts fail, falls back to `shutil.copytree` + `shutil.rmtree` — which works even when a directory handle is held open. See `_resilient_move()` in `obsidian_folder_sort.py`.
