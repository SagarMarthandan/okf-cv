...
**Changes:**
- **New `track_outcomes.py`:** Standalone outcome + channel tracker. `set <company> <status> [--source] [--notes]` records status (`sent/viewed/interview/second_round/offer/rejected/ghosted`) and application source per folder in a new `Application_Status.yaml` (does not touch any pipeline-owned file). `pending` lists every application folder missing a recorded status. `report [--since]` prints response-rate digest by role archetype and by application source. Backfilled with the 4 confirmed interview outcomes located on disk (Ember, Almedia, Project-S, Moss).
- **ATS score gate downgraded from blocking to informational (`01_ats_and_jd_archival.md` §2):** Previously `total_score < 85` set `score_gate_verdict: HOLD` and stopped the pipeline before Step 2. Since the score is a self-assessment by the same model performing the rewrite, with no external ATS validation and no measured correlation to interview outcomes, it now sets `score_gate_verdict: REVIEW` (populating `remedy_suggestions` for visibility) but always proceeds to Step 2. Never blocks submission.
- **Resume summary "4 years at IBM" headline removed (`02_resume_and_visual_audit.md` §"Summary", all 6 base resumes):** The summary previously opened with a standalone "X years of production experience" clause tied to 2014–2018 mainframe/CICS work — honest but domain-mismatched, inviting a false-signal read in the first line. Rewrote all base resume summaries (`resume.md`, `resume_data_engineer.md`, `resume_analytics_engineer.md`, `resume_data_analyst.md`, `resume_ai_data_engineer.md`, `resume_de.md`) to lead with education + a background credential phrase ("enterprise-grade production systems background") instead; the specific "4 years / throughput / uptime" numbers remain in the IBM experience bullets where they belong. Updated the pipeline rule text so future generations don't regress. All 6 summaries re-verified under their character limits (200 EN / 170 DE).
- **Cover letter archetype-conditional AI/RAG content + no-resume-rehash rule (`03_cover_letter.md` §2):** The "active work with local LLMs, RAG, and LangGraph architectures" integration point previously fired unconditionally on every cover letter, producing non-sequiturs on Business Analyst / Data Analyst applications (verified in a live example). Now gated on `role_archetype` being AI Engineer, AI Data Engineer, AI/LLMOps, ML Engineering, Agentic/Automation, or Data Engineering with an explicit AI/ML JD requirement. Added a "No Resume Rehash" rule requiring the letter to carry a "why this company" reason and a first-90-days framing instead of re-listing resume project bullets.
- **Keyword pollution cleanup:** Expanded `okf/noise_words.yaml` with generic tokens the enrichment filter was missing (`data`, `context`, `overview`, `engineering`, `roles`, `services`, `first`, `high`, `bash`, `access`, `tech`, `document`, `analytical`, `based`, and others) that had leaked into production portfolio frontmatter via `okf_learn.py`'s incomplete filter. Removed the already-polluted junk keywords (`data`, `context`, `overview`) from `nyc_taxi_analytics_pipeline_2025.md` and `adventure_works_global_sales_returns_dashboard.md`. Verified via `okf_lint.py` and a full-portfolio grep that no junk tokens remain.
- **German-market coverage added to the OKF matching layer:** `okf/synonyms.yaml` gained ~15 German-term synonym groups (Werkstudent, Praktikum, Berufserfahrung, Kenntnisse, Datenbank, Datenmodellierung, etc.) mapped to existing canonical English keys. `okf/phrase_patterns.yaml` gained 12 German domain-phrase regex patterns (Datenmodell, Datenqualität, Berichtswesen, etc.), all verified to compile. `okf/noise_words.yaml` gained German filler-word coverage (sowie, erforderlich, wünschenswert, idealerweise, etc.) so German JDs don't have common connective/qualifier words extracted as false-positive keywords.
- **Diversity audit bug fix + doc cross-reference fix (`okf_diversity_audit.py`, `01_ats_and_jd_archival.md`):** `referral_count` previously only counted the literal source `"Referral"`, silently excluding `"LinkedIn Connection"` — a valid referral source per the pipeline's own schema — undercounting the true referral rate. Now sums both. Fixed the broken doc pointer from a nonexistent README "Weekly Review" section to the actual location in `docs/ARCHITECTURE.md`, and added a pointer to the new `track_outcomes.py report` command for per-application outcome tracking.
- **Stale project-name mappings corrected (`okf/project_mappings.yaml`):** Several canonical names in the Obsidian normalization rules pointed at project titles that no longer exist in the portfolio (renamed or reorganized since the mappings were last updated): "Master Thesis Subspace Poisoning" → corrected to "COMAD PCA - Robust Principal Component Analysis" (actual file title); "Bike Rentals Data Transformation Pipeline" → corrected to "Chicago Crime & Divvy Bike-Share Data Engineering Pipeline"; "Airbnb Analytics Engineering Pipeline" → corrected to "Airbnb Data Engineering Project (dbt + Snowflake + Dagster)"; "Star Schema for Enterprise Sales Analytics" consolidated into "Adventure Works Global Sales Returns Dashboard" (same underlying project, was creating a phantom duplicate); OKF-CV canonical name corrected to include the `&` matching the actual file title. Verified via direct calls to `normalize_project()`.
- **German city coverage expanded in location lookup (`config.py`):** `JOB_LOCATION_TO_CANDIDATE_CITY` was missing most major German cities outside the four candidate-adjacent metro areas, forcing a web-search fallback on every first encounter (confirmed via `.location_cache.json` showing repeated lookups for Karlsruhe, Mannheim, Heidelberg, Münster, Würzburg, etc.). Added ~20 entries covering Karlsruhe, Mannheim, Heidelberg, Würzburg, Nürnberg, Augsburg, Regensburg, Freiburg, Ludwigsburg, Böblingen, Münster, Krefeld, Dusseldorf, Leipzig, Dresden, Hannover, Bremen, and their common English/transliterated spellings. All verified to resolve without web search.
- **ReportLab contact-header bug fix (`renderers/resume_common.py`):** `generate_reportlab_contact_header()` previously always emitted a third contact line (`visa • availability`) even when both fields were empty, producing a stray bullet line — while the LaTeX renderer already conditionally skipped an empty third line. Rewrote the ReportLab version to build each contact line from non-empty parts only, matching the LaTeX renderer's logic exactly. This only affects output when the ReportFallback renderer is used (primary path or automatic parse-integrity-audit recovery).

**Verification:**
- `okf_lint.py`: PASSED (16 portfolio files clean).
- All 6 patched base resume summaries measured under their character limits (200 EN / 170 DE) after two rounds of tightening.
- Live end-to-end test: regenerated and recompiled the in-flight Trassets GmbH application's `Resume.yaml` → `SAGAR_MARTHANDAN_Lebenslauf.pdf` with the corrected summary; rendered to PNG and visually confirmed 1 page, exact 2-line Zusammenfassung, no layout break, IBM bullets intact.
- `track_outcomes.py`: tested `set`, `pending`, `report` end-to-end; confirmed status-history accumulates correctly across repeated `set` calls without duplicating files; confirmed `pending` correctly flags 298 of 302 folders as unlogged.
- `okf_diversity_audit.py`: ran live post-fix, confirmed output includes both `Referral` and `LinkedIn Connection` in the referral-rate calculation; live run showed 0% referral rate over the last 14 days (51 applications), corroborating the outcome-tracking finding that the referral channel is untested.
- `normalize_project()`: tested all 4 corrected mappings directly, all resolve to the correct canonical project name.
- `nearest_candidate_city()`: tested 9 previously-missing cities (Karlsruhe, Mannheim, Heidelberg, Hannover, Bremen, Leipzig, Dresden, Münster, Nürnberg), all resolve from the static table without a web-search fallback.
- ReportLab contact-header fix: compiled two test resumes (visa/availability both populated, and both empty) via the ReportFallback renderer, rendered both to PNG, visually confirmed 3-line header in the populated case and clean 2-line header (no stray bullet) in the empty case.
- All edited Python files (`track_outcomes.py`, `okf_diversity_audit.py`, `config.py`, `renderers/resume_common.py`) pass `ast.parse()`.
- All edited YAML files (`okf/synonyms.yaml`, `okf/noise_words.yaml`, `okf/phrase_patterns.yaml`, `okf/project_mappings.yaml`) pass `yaml.safe_load()`; all 162 phrase-pattern regexes compile.
