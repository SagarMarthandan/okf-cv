...
- **LaTeX Compilation & Project Format Polish:** Generates a professional LaTeX resume (`SAGAR_MARTHANDAN_Resume.tex` or `SAGAR_MARTHANDAN_Lebenslauf.tex` for German) and converts project listings from standard bullet points into a compact, single-paragraph prose block. Project names are rendered on their own line (`\par`), with the summary starting on the next line. Tools are not displayed in the project header — they remain available only in the YAML for the parseability audit. The LaTeX preamble includes `glyphtounicode` and `hyphenat` safeguards to prevent font ligature corruption and auto-hyphenation in ATS PDF-to-text parsers. (Skipped when `render_mode: reportfallback` — the ReportFallback renderer produces single-paragraph projects automatically.)
...
- **Constraints & Eye-Test Audit:** Runs character-length audits:
  ...
  - Project summaries: Must be `<= 300` characters (`<= 280` characters for German projects), counting only the summary/description text (project name, separator, and link markup are excluded). Must fit within `<= 3` lines.
