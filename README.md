...
- **Zvec Semantic Layer:** All 15 portfolio files are embedded using `all-MiniLM-L6-v2` (384-dim vectors) and stored in a local Zvec database under `okf/zvec_db/`. Incremental re-embedding via content hash detection — only changed files are re-embedded. When `okf_learn.py` adds new keywords, modified files are automatically re-embedded into the Zvec database.
...
- **Dynamic Section Ordering:** Resume section order is data-driven and depends on the `resume_style` key. US style uses `DEFAULT_SECTION_ORDER` in `renderers/resume_common.py` (Summary → Technical Skills → Projects → Professional Experience → Education → Spoken Languages). German style uses `GERMAN_STYLE_SECTION_ORDER` in `renderers/resume_latex_german.py` / `renderers/resume_reportfallback_german.py` (Summary → Professional Experience → Education → Technical Skills → Spoken Languages). German style has no separate Projects section — the 3 JD-aligned projects are folded into Professional Experience as `project_bullets` under an "Independent Data Engineering & Professional Development" entry (rendered in `name --- [GitHub] --- summary` format with quantified metrics), plus a 4th plain-text bullet for other skills/tools. The entry date ends at April 2025 (candidate is now studying economics). The title uses a concrete role (e.g., `Data Engineer`, `Analytics Engineer`) — never `Architect`/`Lead`/`Manager`. A resume YAML may override the order per-application by supplying a top-level `section_order` key; unknown keys are ignored, omitted sections are skipped.
