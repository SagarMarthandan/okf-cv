...
- **Embedding Daemon:** [embedding_server.py](embedding_server.py) is a local TCP daemon (127.0.0.1, ports 54321-54325) that holds the `all-MiniLM-L6-v2` model in memory. The pipeline invokes `zvec_hybrid_search.py` 3 times per run (pre-rewrite similarity, hybrid portfolio search, post-rewrite similarity) — without the daemon, each invocation loads the model fresh (~21s on CPU, ~63s total wasted). The daemon is auto-started by `zvec_hybrid_search.py` on first use and auto-shuts down after 30 min of inactivity. Falls back to direct model loading if the daemon is unavailable (pipeline still works, just slower). Manual control: `python embedding_server.py --status` / `--stop`. State: `okf/.embedding_server.json`, log: `okf/.embedding_server.log`.
...
- **Skill Gap Analysis (P2):** Extracts required skills from the JD and cross-references against the base resume and matched projects. Stores gaps as a `skill_gaps` list in `ATS_Report.yaml` for targeted closure in Step 2.
- **Contextual Placement Weighting (P4):** Checks which resume sections (skills, projects, experience) contain each critical JD keyword. Applies placement multipliers (1.0x skills, 1.2x projects, 1.3x experience, 1.5x multiple). Stored as `placement_breakdown` in `ATS_Report.yaml`.
- **Pre-Rewrite Semantic Similarity (P1):** Computes cosine similarity between the base resume and JD using `resume_jd_similarity.py` (all-MiniLM-L6-v2 embeddings). Stored as `resume_jd_semantic_similarity.pre_rewrite_similarity` for before/after tracking.
...
- **Render Mode & Resume Style Selection:** Before compilation, the user chooses between `LaTeX` (pdflatex, `.tex` source preserved, optional prose refinement) and `ReportFallback` (ReportLab with LM Roman 10 font, no `.tex` file, single-paragraph projects rendered automatically). The user also chooses between `US Style` (US-convention section order) and `German Style` (Lebenslauf convention with Date & Signature block). Both choices are stored as `render_mode` and `resume_style` keys in `Resume.yaml`.
...
- **Dynamic Section Ordering:** Resume section order is data-driven and depends on the `resume_style` key. US style uses `DEFAULT_SECTION_ORDER` in `renderers/resume_common.py` (Summary → Technical Skills → Projects → Professional Experience → Education → Spoken Languages). German style uses `GERMAN_STYLE_SECTION_ORDER` in `renderers/resume_latex_german.py` / `renderers/resume_reportfallback_german.py` (Summary → Professional Experience → Education → Technical Skills → Spoken Languages → Date & Signature). German style has no separate Projects section — the 3 JD-aligned projects are folded into Professional Experience as `project_bullets` under an "Independent Data Engineering & Professional Development" entry (rendered in `name --- [GitHub] --- summary` format with quantified metrics), plus a 4th plain-text bullet for other skills/tools. The entry date ends at April 2025 (candidate is now studying economics). The title uses a concrete role (e.g., `Data Engineer`, `Analytics Engineer`) — never `Architect`/`Lead`/`Manager`. The Date & Signature block renders with no section header as `City, den DD.MM.YYYY` / signature line / `Sagar Marthandan`. A resume YAML may override the order per-application by supplying a top-level `section_order` key; unknown keys are ignored, omitted sections are skipped.
- **Project Verification Links:** Reads `Repo:` lines from `project_info.md` and copies them into each project block in `Resume.yaml` as `repo_url:`. These are compiled as clickable `[GitHub]` links in the PDF — both in the LaTeX output (`\href{repo_url}{\color{darkblue}\small[GitHub]}`) and the ReportLab fallback (`<a href='...'>[GitHub]</a>`).
- **LaTeX Compilation & Project Format:** Generates a professional LaTeX resume (`SAGAR_MARTHANDAN_Resume.tex` or `SAGAR_MARTHANDAN_Lebenslauf.tex` for German). Projects are rendered in the `name --- [GitHub] --- summary` single-paragraph format directly by the renderer (YAML `bullets` joined into prose). The project name, em-dash separators (`---`), and link markup (`[GitHub]`) are excluded from the character count — only the summary text counts toward the `<= 300` char (English) / `<= 280` char (German) limit. Tools are not displayed in the project header — they remain in the YAML for the parseability audit. The LaTeX preamble includes `glyphtounicode` and `hyphenat` safeguards to prevent font ligature corruption and auto-hyphenation in ATS PDF-to-text parsers. (The ReportFallback renderer produces the same `name --- [GitHub] --- summary` format automatically.)
...
- **Post-Rewrite ATS Rescoring:** Updates `post_rewrite_ats_score` in `ATS_Report.yaml` and recompiles `ATS_Report.pdf`. Also computes post-rewrite cosine similarity between the optimized resume and JD via `resume_jd_similarity.py`, storing it as `post_rewrite_similarity` for before/after alignment tracking.
...
- **ATS Score Delta Tracking (P3):** Each learning log entry now includes `pre_rewrite_ats_score` and `post_rewrite_ats_score` extracted from `ATS_Report.yaml`, creating a longitudinal dataset of which keyword enrichments and project selections correlate with the biggest ATS score improvements.
...
│       │   ├── resume_latex_german.py    # Resume LaTeX renderer (German style: Lebenslauf section order + date/signature)
...
See [docs/CHANGELOG.md](docs/CHANGELOG.md) for the full version history (v1–v28.14).
